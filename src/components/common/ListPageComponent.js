...
const ListPageComponent = ({ movePage, start, end, prev, next, pageNums,page }) => {
    ...
    return (
        <div className="flex m-4 p-2 justify-center">
            ...
            <ul className="flex">
                {prev ? <li className="mb-2 p-2 bg-blue-500 border-2 font-bold rounded-lg"
                    onClick={() => handleClickPage(start - 1)}
                >PREV</li> : <></>}
                {pageNums.map(num => <li
                    className="mb-2 p-2 bg-blue-500 border-2 font-bold rounded-lg"
                ...
                {next ? <li className="mb-2 p-2 bg-blue-500 border-2 font-bold rounded-lg"
